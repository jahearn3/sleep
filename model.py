...
# df['rating_encoded'] = df['rating_smartwatch'].map(
...
# Convert hour finished eating/screentime by to time interval between
#     eating/screentime and sleep time
df['hours_between_eat_and_sleep'] = (
    12 + df['start_time_hr'] - df['hour_finished_eating_by']
)
df['hours_between_screen_and_sleep'] = (
    12 + df['start_time_hr'] - df['hour_finished_screen_time_by']
)
...
target_correlation = correlation_matrix['score_smartwatch'] \
    .drop('score_smartwatch')  # Drop self-correlation
